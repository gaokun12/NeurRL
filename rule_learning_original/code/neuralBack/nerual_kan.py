...
from dforl_torch import RuleLayer
...
class ConjunctionLayer(nn.Module):
    def __init__(self, in_size):
        ...
        self.alpha = 10
        weights = torch.Tensor(1, self.size_in)
    ...
    def forward(self,x):
        ...
        conjuntor = torch.mm(x, self.interpretable_rul_weights.t())
        activated_values = nn.functional.relu(conjuntor)
        return activated_values
    
    def constrains(self):
        loss_r1 = torch.sum(torch.abs(self.weights))    

class Disjunctor(nn.Module):
    def __init__(self, in_size):
        super().__init__()
        self.size_in = in_size
        self.alpha = 10
        weights = torch.Tensor(1, self.size_in)
        self.weights = nn.init.normal_(nn.Parameter(weights, requires_grad=True), mean=0.5, std = 0.1)

        

class MultipleRuleLayer(nn.Module):   
    def __init__(self, in_size, rule_number):
        super().__init__()
        self.rule1 = ConjunctionLayer(in_size=in_size)
        self.rule2 = ConjunctionLayer(in_size=in_size)
        self.rule_or = RuleLayer(in_size=2, rule_number=rule_number)
    def forward(self, x):
        x1 = self.rule1(x)
        x2 = self.rule2(x)
        rule_input = torch.cat([x1, x2], dim=1)
        x = self.rule_or(rule_input)
        return x
...
def train_model(model, trainloader, testloader, epoch):
...
if __name__=='__main__':
    ...
    model = NeuralPredicate(number_features=3, number_predicate=1, number_rules=1)  
    args = model.args
    if args.cuda:
